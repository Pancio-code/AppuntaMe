...
class AppuntamentiPage extends StatefulWidget {
  @override
  _AppuntamentiPageState createState() => _AppuntamentiPageState();
}

class _AppuntamentiPageState extends State<AppuntamentiPage> {
  final dbClient = DatabaseClient.instance;
  Iterable<Contact> _contacts ;
  TextEditingController numberController = new TextEditingController();
  TextEditingController nameController = new TextEditingController();
  TextEditingController serviziController = new TextEditingController();
  TextEditingController dateController = new TextEditingController();
  TextEditingController mailController = new TextEditingController();
  TextEditingController prezzoController = new TextEditingController();
                      ...
                      child: SimpleAutocompleteFormField<Contact>(
                        ...
                        itemToString: (contact) {
                          if(contact != null && contact.displayName == nameController.text) {
                            for(Map<String, dynamic> item in list_of_client) {
                              if(item['phoneNumber'] == flattenPhoneNumber(HelperFunctions.getValidPhoneNumber(contact.phones))) {
                                mailController.text = item['email'];
                                serviziController.text = item['Servizio'];
                                prezzoController.text = item['prezzo'].toString();
                                break;
                              }
                            }
                          ...
                          child:ElevatedButton.icon(
                            ...
                            onPressed: () async{ 
                              if(_formKey.currentState.validate()) {
                                ...
                                _sendSMS("Confermato appuntamento per il " + dateController.text + (serviziController.text.length > 2 ? "\nServizio: " + serviziController.text : "") + (prezzoController.text != '' ? "\nPrezzo: " + prezzoController.text + "€" : "") + (number_saved != null ? "\nPer eventuali modifiche chiamare: 3888865279" : ''), [numberController.text]);
                                
                                currentClient = new Appuntamento(nameController.text.hashCode + numberController.text.hashCode + dateController.text.hashCode, nameController.text, numberController.text,mailController.text,dateController.text,serviziController.text,prezzoController.text,int.parse(key.data));
                                bool esiste = false;
                            ...
                            onPressed: () async{ 
                              if(_formKey.currentState.validate()) {
                                ...
                                if(mailController.text != '') 
                                  await Share.share("Confermato appuntamento per il " + dateController.text + (serviziController.text.length > 2 ? "\nServizio: " + serviziController.text : "") + (prezzoController.text != '' ? "\nPrezzo: " + prezzoController.text + "€" : "")+ (number_saved != null ? "\nPer eventuali modifiche chiamare: 3888865279" : ''));
                                else 
  ...
  void _insert_client(Map<String, dynamic> row) async {
      ...
      buttons: [
        DialogButton(
          ...
          onPressed: () {
            bool esiste = false;
            for(Map<String, dynamic> item in list_of_client) {
              if(item['phoneNumber'] == currentClient.phoneNumber) {
                esiste = true;
                break;
              }
            }
            if(!esiste) _insert_client(Client(nameController.text.hashCode + numberController.text.hashCode + mailController.text.hashCode + serviziController.text.hashCode + prezzoController.text.hashCode, nameController.text, numberController.text,mailController.text,serviziController.text,prezzoController.text).toMap());        
